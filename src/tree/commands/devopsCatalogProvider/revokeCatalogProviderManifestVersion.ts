    ...
    vscode.commands.registerCommand(
      ...
      async (item: DevOpsTreeItem) => {
        ...
        const confirmation = await YesNoQuestion(
          `Are you sure you want to revoke the catalog provider manifest${versionId !== "" ? " version" : ""}? ${
            item.name
          }?`
        );
        ...
        await vscode.window.withProgress(
          ...
          async () => {
            ...
            await DevOpsService.revokeCatalogManifest(
              provider,
              manifest.name,
              ...
              LogService.error(
                `Error revoking manifest${versionId !== "" ? " version" : ""} ${item.name} on provider ${
                  provider.name
                }`,
                error
              );
            ...
            vscode.window.showInformationMessage(
              `Manifest ${versionId !== "" ? " version" : ""} ${item.name} was revoked on provider ${provider.name}`
            );
          }
        );
      }
