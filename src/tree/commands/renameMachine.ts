  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(CommandsFlags.treeRenameVm, async (item: VirtualMachineTreeItem) => {
      ...
      if (newVmName) {
        ...
        await ParallelsDesktopService.renameVm(item.id, newVmName).then((result) => {
          if (!result) {
          ...
          LogService.sendTelemetryEvent(TelemetryEventIds.VirtualMachineAction, `Virtual Machine ${item.name} renamed to ${newVmName}`);  
        }).catch(reject => {
          vscode.window.showErrorMessage(`Failed to rename ${item.name} to ${newVmName}`);
