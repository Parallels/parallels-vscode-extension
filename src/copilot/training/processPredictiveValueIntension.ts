...
export async function processPredictiveValueIntension(userPrompt: string, valuesToDeductFrom: string[],context: vscode.ChatContext, model: vscode.LanguageModelChat, token: vscode.CancellationToken): Promise<string> {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const response = data.replace(/`/g, '').replace(/{/g, '').replace(/}/g, '').replace(/```/g, '').replace(/\n/g, '').trim()
      resolve(response)
    } catch (error) {
      console.error(error)
      reject(error)
