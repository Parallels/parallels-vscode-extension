...
export async function remoteHostIntensionHandler(
    ...
    try {
      ...
      for (const key in remoteHostIntensions) {
        switch (remoteHostIntensions[key].operation.toUpperCase()) {
          case "COUNT": {
            const statusResponse = await remoteHostCountIntensionHandler(
              remoteHostIntensions[key].operation_value,
              stream,
              model,
              token
            );
          ...
          case "LIST_VMS": {
            ...
            const statusResponse = await remoteHostListIntensionHandler(
              remoteHostIntensions[key].target,
              remoteHostIntensions[key].operation_value,
              context,
              stream,
              model,
          ...
          case "STATUS": {
            ...
            const statusResponse = await remoteHostStatusIntensionHandler(
              remoteHostIntensions[key].target,
              context,
              stream,
              model,
