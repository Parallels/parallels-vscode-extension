...
const registerDevOpsManagementProviderRemoveUserClaimCommand = (context: vscode.ExtensionContext, provider: DevOpsRemoteHostsProvider | DevOpsCatalogProvider) => {
  context.subscriptions.push(
    vscode.commands.registerCommand(CommandsFlags.devopsRemoteProviderManagementRemoveUserClaim, async (item: any) => {
      ...
      const confirmation = await YesNoQuestion(
        `Are you sure you want to remove ${item.name} claim from user?`
      );
      ...
      DevOpsService.testHost(provider).then(async () => {
        ...
        await DevOpsService.removeRemoteHostUserClaim(provider, userId, item.name).catch(() => {
          vscode.window.showErrorMessage(`Failed to remove claim ${item.name} for user ${user.name} on the remote provider ${provider?.name ?? 'Unknown'}`);
          foundError = true;
          return;
        })
        ...
        vscode.window.showInformationMessage(`Claim ${item.name} for user ${user.name} was deleted successfully on the remote provider ${provider?.name ?? 'Unknown'}`);
        if (item.className === 'DevOpsRemoteHostProvider') {
