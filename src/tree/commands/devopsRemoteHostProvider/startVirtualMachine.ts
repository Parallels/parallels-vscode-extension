...
const registerDevOpStartVirtualMachineCommand = (
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(CommandsFlags.devopsStartRemoteProviderHostVm, async (item: DevOpsTreeItem) => {
      ...
      telemetry.sendOperationEvent(TELEMETRY_DEVOPS_REMOTE, "START_VIRTUAL_MACHINE_COMMAND_CLICK");
      if (!item) {
        return;
      }
      vscode.window.withProgress(
        ...
        async () => {
          ...
          if (!ok || foundError) {
            ShowErrorMessage(TELEMETRY_DEVOPS_REMOTE, `Failed to start virtual machine ${item.name}`, true);
            return;
          }
