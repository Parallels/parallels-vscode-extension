...
const registerDevOpsTaintCatalogProviderManifestVersionCommand = (context: vscode.ExtensionContext, provider: DevOpsCatalogProvider) => {
  context.subscriptions.push(
    vscode.commands.registerCommand(CommandsFlags.devopsTaintCatalogProviderManifestVersion, async (item: DevOpsTreeItem) => {
      if (!item) {
        return;
      }
      ...
      const confirmation = await YesNoQuestion(
        `Are you sure you want to taint the catalog provider manifest${versionId !== '' ? ' version':''}? ${item.name}?`
      );
      ...
      await vscode.window.withProgress(
        ...
        async () => {
          ...
          await DevOpsService.taintCatalogManifest(provider, manifest.name, manifestItem.version, manifestItem.architecture).catch((error) => {
            LogService.error(`Error tainting manifest${versionId !== '' ? ' version':''} ${item.name} on provider ${provider.name}`, error);
          ...
          vscode.window.showInformationMessage(`Manifest ${versionId !== '' ? ' version':''} ${item.name} was tainted on provider ${provider.name}`);
        });
    })
  );
};
