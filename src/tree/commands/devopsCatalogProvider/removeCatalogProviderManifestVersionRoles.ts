...
const registerDevOpsRemoveRoleFromCatalogProviderManifestVersionCommand = (
  context: vscode.ExtensionContext,
  provider: DevOpsCatalogProvider
) => {
  context.subscriptions.push(
    vscode.commands.registerCommand(
      CommandsFlags.devopsRemoveRoleFromCatalogProviderManifestVersion,
      async (item: any) => {
        const telemetry = Provider.telemetry();
        telemetry.sendOperationEvent(
          TELEMETRY_DEVOPS_CATALOG,
          "REMOVE_CATALOG_PROVIDER_MANIFEST_VERSION_ROLE_COMMAND_CLICK"
        );
        if (!item) {
          return;
        }
        ...
        DevOpsService.testHost(provider)
          .then(async () => {
            ...
            await DevOpsService.removeCatalogManifestRoles(
              provider,
              manifest.name,
              ...
              ShowErrorMessage(
                TELEMETRY_DEVOPS_CATALOG,
                `Failed to remove role ${roles.join(",")} from the catalog manifest ${
            ...
            vscode.window.showInformationMessage(
              `Role ${roles.join(",")} was removed successfully from the catalog manifest ${
                manifestItem.catalog_id
              } version ${manifestItem.version} for ${manifestItem.architecture}`
            );
