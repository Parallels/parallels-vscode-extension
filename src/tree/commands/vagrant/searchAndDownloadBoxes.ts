...
export function registerVagrantSearchAndDownloadCommand(context: vscode.ExtensionContext, provider: VagrantBoxProvider) {
    ...
    vscode.commands.registerCommand(CommandsFlags.vagrantSearchAndDownload, async () => {
      return new Promise((resolve, reject) => {
        try {
              ...
              quickPick.onDidChangeSelection(async (selection) => {
                ...
                vscode.window.showInputBox({
                  ...
                  vscode.window.withProgress(
                    ...
                    async () => {
                      const box = await VagrantService.init(selection[0].label, boxName ?? selection[0].label, false, context).catch(reject => {
                        LogService.error(`Failed to create Vagrant Box ${boxName}: ${reject}`, "CreateDockerContainerCommand");
                        LogService.sendTelemetryEvent(TelemetryEventIds.VirtualMachineAction, `Failed to create Vagrant Box ${boxName}`);
          ...
          vscode.window.showErrorMessage(`Failed to download and create Vagrant Box from cloud`);
          LogService.error(`Failed to download and create Vagrant Box from cloud: ${error}`, "CreateDockerContainerCommand");
          LogService.sendTelemetryEvent(
            TelemetryEventIds.VirtualMachineAction,
