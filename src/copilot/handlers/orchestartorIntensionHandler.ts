...
export async function orchestratorIntensionHandler(
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const orchestratorProviderIntensions = await processOrchestratorProviderIntensions(
        userIntension,
        context,
        model,
        token
      ...
      for (const key in orchestratorProviderIntensions) {
        switch (orchestratorProviderIntensions[key].operation.toUpperCase()) {
          case "COUNT": {
            const statusResponse = await orchestratorCountIntensionHandler(
              orchestratorProviderIntensions[key].operation_value,
              stream,
              model,
              token
            );
          ...
          case "LIST_HOSTS": {
            ...
            const statusResponse = await orchestratorListIntensionHandler(
              orchestratorProviderIntensions[key].target,
              orchestratorProviderIntensions[key].operation,
              orchestratorProviderIntensions[key].operation_value,
              context,
              stream,
              model,
          ...
          case "STATUS": {
            ...
            const statusResponse = await orchestratorStatusIntensionHandler(
              orchestratorProviderIntensions[key].target,
              context,
              stream,
              model,
