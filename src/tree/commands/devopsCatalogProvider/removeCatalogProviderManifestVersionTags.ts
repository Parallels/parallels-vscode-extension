...
const registerDevOpsRemoveTagFromCatalogProviderManifestVersionCommand = (context: vscode.ExtensionContext, provider: DevOpsCatalogProvider) => {
  context.subscriptions.push(
    vscode.commands.registerCommand(CommandsFlags.devopsRemoveTagFromCatalogProviderManifestVersion, async (item: any) => {
      if (!item) {
        return;
      }
      ...
      DevOpsService.testHost(provider).then(async () => {
        ...
        await DevOpsService.removeCatalogManifestTags(provider, manifest.name, manifestItem.version, manifestItem.architecture, tag).catch(() => {
          vscode.window.showErrorMessage(`Failed to remove tag ${tag.join(",")} from the catalog manifest ${manifestItem.catalog_id} version ${manifestItem.version} for ${manifestItem.architecture}`);
          foundError = true;
          return;
        })
        ...
        vscode.window.showInformationMessage(`Tag ${tag.join(",")} was removed successfully from the catalog manifest ${manifestItem.catalog_id} version ${manifestItem.version} for ${manifestItem.architecture}`);
