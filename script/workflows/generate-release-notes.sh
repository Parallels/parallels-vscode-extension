...
LAST_RELEASE_MERGED_AT=$(gh pr view $LAST_RELEASE_PR --repo Parallels/parallels-vscode-extension --json mergedAt | jq -r '.mergedAt')

CHANGELIST=$(gh pr list --repo Parallels/parallels-vscode-extension --base main --state merged --json title --search "merged:>$LAST_RELEASE_MERGED_AT -label:no-release")
