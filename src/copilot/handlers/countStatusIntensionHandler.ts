...
export async function countStatusIntensionHandler(operation: string,stream: vscode.ChatResponseStream, model: vscode.LanguageModelChat, token: vscode.CancellationToken ): Promise <CopilotOperation> {
    ...
    try {
      ...
      switch (operation.toUpperCase()) {
        case 'RUNNING': {
          ...
          response.operation = `There ${runningVms.length > 1 ? 'are' : 'is'} ${runningVms.length} machines running:\n - ${runningVms.map(vm => vm.Name).join('\n - ')}`;
          response.state = 'success';
          resolve(response);
          break;
        }
        case 'STOPPED': {
          ...
          response.operation = `There ${stoppedVms.length > 1 ? 'are' : 'is'} ${stoppedVms.length} machines stopped:\n - ${stoppedVms.map(vm => vm.Name).join('\n - ')}`;
          response.state = 'success';
          resolve(response);
          break;
        }
        case 'PAUSED': {
          ...
          response.operation = `There ${pausedVms.length > 1 ? 'are' : 'is'} ${pausedVms.length} machines paused:\n - ${pausedVms.map(vm => vm.Name).join('\n - ')}`;
          response.state = 'success';
          resolve(response);
          break;
        }
        case 'SUSPENDED': {
          ...
          response.operation = `There ${suspendedVms.length > 1 ? 'are' : 'is'} ${suspendedVms.length} machines suspended:\n - ${suspendedVms.map(vm => vm.Name).join('\n - ')}`;
          response.state = 'success';
          resolve(response);
          break;
        }
