...
const registerDevOpsManagementProviderAddRoleCommand = (context: vscode.ExtensionContext, provider: DevOpsRemoteHostsProvider | DevOpsCatalogProvider) => {
    ...
    vscode.commands.registerCommand(CommandsFlags.devopsRemoteProviderManagementAddRole, async (item: any) => {
      ...
      DevOpsService.testHost(provider).then(async () => {
        ...
        await DevOpsService.createRemoteHostRole(provider, request).catch(() => {
          vscode.window.showErrorMessage(`Failed to add role ${name} to the remote host ${provider?.name ?? 'Unknown'}`);
          foundError = true;
          return;
        })
        ...
        vscode.window.showInformationMessage(`Role ${name} was added successfully to the remote host ${provider?.name ?? 'Unknown'}`);
        if (item.className === 'DevOpsRemoteHostProvider') {
