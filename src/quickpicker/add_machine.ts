...
export async function addVirtualMachineInput(guestOs: string, context: ExtensionContext) {
    ...
    const availableImages: VirtualMachineImage[] = [
        ...
        },
        {
            ...
            bootCommand: ['<wait>e<wait><down><down><down><end><wait> autoinstall ds=nocloud-net\\\\;s=http://{{.HTTPIP}}:{{.HTTPPort}}/ubuntu/<f10><wait>'],
            bootWait: '10s',
            cpuCount: 2,
            memorySize: 4092,
            diskSize: 65535,
    ...
    const availableDistributions = [...new Set(availableImages.filter((image) => image.base === guestOs).map((image) => image.distro))];
    ...
    async function pickPlatform(input: MultiStepInput, state: Partial<State>) {
        ...
        const platforms = availableImages.filter((image) => image.distro === distro && image.base === guestOs).map((image) => ({ label: image.platform }));
        const pick = await input.showQuickPick({
            title,
            step: 2,
            totalSteps: 5,
