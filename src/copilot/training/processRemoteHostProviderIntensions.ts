...
import { RemoteHostProviderIntensionPrompt } from './prompts/remoteHostProviderPrompt';
import { LogIntensionPrompt } from './debug';

export async function processRemoteHostProviderIntensions(userPrompt: string, context: vscode.ChatContext, model: vscode.LanguageModelChat, token: vscode.CancellationToken): Promise<CopilotUserIntension[]> {
  return new Promise(async (resolve, reject) => {
    try {
      const { messages } = await renderPrompt(
        RemoteHostProviderIntensionPrompt,
