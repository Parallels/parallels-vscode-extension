...
const registerDevOpsManagementProviderRemoveUserCommand = (context: vscode.ExtensionContext, provider: DevOpsRemoteHostsProvider | DevOpsCatalogProvider) => {
    ...
    vscode.commands.registerCommand(CommandsFlags.devopsRemoteProviderManagementRemoveUser, async (item: any) => {
      ...
      DevOpsService.testHost(provider).then(async () => {
        ...
        await DevOpsService.removeRemoteHostUsers(provider, userId).catch(() => {
          vscode.window.showErrorMessage(`Failed to delete user ${user.name} on the Remote Provider ${provider?.name ?? 'Unknown'}`);
          foundError = true;
          return;
        })
        ...
        vscode.window.showInformationMessage(`User ${user.name} was deleted successfully on the Remote Provider ${provider?.name ?? 'Unknown'}`);
        if (item.className === 'DevOpsRemoteHostProvider') {
