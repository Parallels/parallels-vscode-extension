...
ESCAPED_URL=$(echo "$PARALLELS_CATALOG_URL" | sed 's/\//\\\//g')
sed -i "s/export const PARALLELS_CATALOG_URL = \".*\"/export const PARALLELS_CATALOG_URL = \"$ESCAPED_URL\"/g" ./src/services/configurationService.ts
