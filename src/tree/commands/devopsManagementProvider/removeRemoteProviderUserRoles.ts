...
const registerDevOpsManagementProviderRemoveUserRoleCommand = (context: vscode.ExtensionContext, provider: DevOpsRemoteHostsProvider | DevOpsCatalogProvider) => {
  context.subscriptions.push(
    vscode.commands.registerCommand(CommandsFlags.devopsRemoteProviderManagementRemoveUserRole, async (item: any) => {
      ...
      const confirmation = await YesNoQuestion(
        `Are you sure you want to remove ${item.name} role from user?`
      );
      ...
      DevOpsService.testHost(provider).then(async () => {
        ...
        await DevOpsService.removeRemoteHostUserRole(provider, userId, item.name).catch(() => {
          vscode.window.showErrorMessage(`Failed to remove role ${item.name} for user ${user.name} on the remote provider ${provider?.name ?? 'Unknown'}`);
          foundError = true;
          return;
        })
        ...
        vscode.window.showInformationMessage(`Role ${item.name} for user ${user.name} was deleted successfully on the remote provider ${provider?.name ?? 'Unknown'}`);
        if (item.className === 'DevOpsRemoteHostProvider') {
