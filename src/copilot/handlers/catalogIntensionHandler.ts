...
export async function catalogIntensionHandler(intension: CopilotUserIntension,context: vscode.ChatContext, stream: vscode.ChatResponseStream, model: vscode.LanguageModelChat, token: vscode.CancellationToken ): Promise <CopilotOperation> {
    ...
    try {
      ...
      for (const key in catalogProviderIntensions) {
        switch (catalogProviderIntensions[key].operation.toUpperCase()) {
          case 'COUNT': {
            const statusResponse = await catalogCountIntensionHandler(catalogProviderIntensions[key].operation_value, stream, model, token);
          ...
          case 'LIST_MANIFESTS': {
            const statusResponse = await catalogListIntensionHandler(catalogProviderIntensions[key].target, catalogProviderIntensions[key].operation_value,context, stream, model, token);
            resolve(statusResponse);
            break;
          }
          case 'STATUS': {
            ...
            const statusResponse = await catalogStatusIntensionHandler(catalogProviderIntensions[key].target,context, stream, model, token);
            resolve(statusResponse);
            break;
          }
          default: {
