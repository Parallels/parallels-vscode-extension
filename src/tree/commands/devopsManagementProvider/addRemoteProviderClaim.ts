...
const registerDevOpsManagementProviderAddClaimCommand = (context: vscode.ExtensionContext, provider: DevOpsRemoteHostsProvider | DevOpsCatalogProvider) => {
    ...
    vscode.commands.registerCommand(CommandsFlags.devopsRemoteProviderManagementAddClaim, async (item: any) => {
      ...
      DevOpsService.testHost(provider).then(async () => {
        ...
        await DevOpsService.createRemoteHostClaim(provider, request).catch(() => {
          vscode.window.showErrorMessage(`Failed to add claim ${name} to the remote host ${provider?.name ?? 'Unknown'}`);
          foundError = true;
          return;
        })
        ...
        vscode.window.showInformationMessage(`Claim ${name} was added successfully to the remote host ${provider?.name ?? 'Unknown'}`);
        if (item.className === 'DevOpsRemoteHostProvider') {
