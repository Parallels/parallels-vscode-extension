...
const registerDevOpsManagementProviderRemoveRoleCommand = (context: vscode.ExtensionContext, provider: DevOpsRemoteHostsProvider | DevOpsCatalogProvider) => {
  context.subscriptions.push(
    vscode.commands.registerCommand(CommandsFlags.devopsRemoteProviderManagementRemoveRole, async (item: any) => {
      ...
      const confirmation = await YesNoQuestion(
        `Are you sure you want to remove ${item.name} role from the host?`
      );
      ...
      DevOpsService.testHost(provider).then(async () => {
        ...
        await DevOpsService.removeRemoteHostRole(provider, roleId).catch(() => {
          vscode.window.showErrorMessage(`Failed to remove role ${item.name} from the remote provider ${provider?.name ?? 'Unknown'}`);
          foundError = true;
          return;
        })
        ...
        vscode.window.showInformationMessage(`Role ${item.name} was deleted successfully from the remote provider ${provider?.name ?? 'Unknown'}`);
        if (item.className === 'DevOpsRemoteHostProvider') {
