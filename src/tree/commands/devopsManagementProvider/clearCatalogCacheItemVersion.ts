...
const registerDevOpsManagementClearCatalogCacheItemVersionCommand = (
  context: vscode.ExtensionContext,
  provider: DevOpsRemoteHostsProvider | DevOpsCatalogProvider
) => {
  const localProvider = provider;
  context.subscriptions.push(
    vscode.commands.registerCommand(CommandsFlags.catalogCacheClearCacheItemVersion, async (item: any) => {
      const telemetry = Provider.telemetry();
      const providerName =
        localProvider instanceof DevOpsCatalogProvider ? TELEMETRY_DEVOPS_CATALOG : TELEMETRY_DEVOPS_REMOTE;
      telemetry.sendOperationEvent(providerName, "CLEAR_CATALOG_CACHE_ITEM_VERSION_COMMAND_CLICK");
      if (!item) {
        return;
      }
      ...
      if (
        ...
        (item.type === "management.remote_hosts.orchestrator.catalog.cache.manifests.manifest.version" ||
          item.type === "management.remote_hosts.orchestrator.host.catalog.cache.manifests.manifest.version")
      ) {
      ...
      await DevOpsService.clearHostCatalogCache(provider, host_id, catalogId, version).catch(() => {
        ShowErrorMessage(providerName, `Failed to clear catalog cache item version on ${hostName}`, true);
        foundError = true;
        return;
      });
      ...
      vscode.window.showInformationMessage(`Catalog cache item version was removed from ${hostName}`);
      if (item.className === "DevOpsRemoteHostProvider") {
...
export const DevOpsManagementClearCatalogCacheItemVersionCommand: DevOpsRemoteProviderManagementCommand = {
  register: registerDevOpsManagementClearCatalogCacheItemVersionCommand
};
