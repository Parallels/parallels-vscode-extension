...
const registerDevOpsRemoveClaimFromCatalogProviderManifestVersionCommand = (context: vscode.ExtensionContext, provider: DevOpsCatalogProvider) => {
  context.subscriptions.push(
    vscode.commands.registerCommand(CommandsFlags.devopsRemoveClaimFromCatalogProviderManifestVersion, async (item: any) => {
      if (!item) {
        return;
      }
      ...
      DevOpsService.testHost(provider).then(async () => {
        ...
        await DevOpsService.removeCatalogManifestClaims(provider, manifest.name, manifestItem.version, manifestItem.architecture, claims).catch(() => {
          vscode.window.showErrorMessage(`Failed to remove claim ${claims.join(",")} from the catalog manifest ${manifestItem.catalog_id} version ${manifestItem.version} for ${manifestItem.architecture}`);
          foundError = true;
          return;
        })
        ...
        vscode.window.showInformationMessage(`Claim ${claims.join(",")} was removed successfully from the catalog manifest ${manifestItem.catalog_id} version ${manifestItem.version} for ${manifestItem.architecture}`);
