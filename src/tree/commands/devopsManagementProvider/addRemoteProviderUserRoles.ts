...
const registerDevOpsManagementProviderAddUserRoleCommand = (context: vscode.ExtensionContext, provider: DevOpsRemoteHostsProvider | DevOpsCatalogProvider) => {
      ...
      for (const role of selected ?? []) {
        DevOpsService.testHost(provider).then(async () => {
          ...
          await DevOpsService.addRemoteHostUserRole(provider, userId, role.label).catch(() => {
            vscode.window.showErrorMessage(`Failed to add role ${role.label} to ${user.name} on the remote host ${provider?.name ?? 'Unknown'}`);
            foundError = true;
            return;
          })
          ...
          vscode.window.showInformationMessage(`Role ${role.label} was added successfully to ${user.name} on the remote host ${provider?.name ?? 'Unknown'}`);
        if (item.className === 'DevOpsRemoteHostProvider') {
